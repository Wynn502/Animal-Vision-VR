...
public class SnakeMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((PositionCurrentFrameLeftHand.y > PositionPreviousFrameLeftHand.y && PositionCurrentFrameRightHand.y > PositionPreviousFrameRightHand.y) ||
            (PositionCurrentFrameLeftHand.y < PositionPreviousFrameLeftHand.y && PositionCurrentFrameRightHand.y < PositionPreviousFrameRightHand.y))
        {
            // aggregate to get hand speed
            HandSpeed = ((leftHandDistanceMoved - playerDistanceMoved) + (rightHandDistanceMoved - playerDistanceMoved));
            ...
            if (Time.timeSinceLevelLoad > 1f)
            {
                transform.position += ForwardDirection.transform.forward * HandSpeed * Speed * Time.deltaTime* -1f;
            }
        }
